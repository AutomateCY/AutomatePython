...
def numFile(saveDir, automaton, filename,
            mode=1):  # /!\mode=1 changes name of automaton/!\ search for the right number to add to save file name.
    num = 1  # number to add
    filename = saveDir + "/" + automaton["name"] + " (" + str(
        num) + ").json"  # name generation: 'saveDir/automaton (num).json'
...
def nameFile(automaton, saveDir, mode,
             filename=""):  # fonction used in saveAutomation(). generates filename and handles name conflicts.
